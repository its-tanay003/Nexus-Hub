    ...
    <div className="relative pl-6 pb-8 border-l-2 border-gray-200 last:border-0 last:pb-0 group">
      ...
      `}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-50 text-sm">
           ...
           {/* Teacher Contact */}
           <div className="flex items-center gap-2 text-gray-700 group/teacher relative">
             <div className="p-1.5 bg-gray-100 rounded-md text-gray-500">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
             </div>
             <div>
                <span className="block text-xs text-gray-400">Teacher</span>
                <a href={`mailto:${schedule.teacher.email}`} className="font-semibold hover:text-blue-600 transition-colors flex items-center gap-1 group/link">
        ...
        {schedule.notes && (
            <div className="mt-4">
                <button 
                  ...
                  className={`w-full flex items-center justify-between text-xs font-bold border rounded-lg p-2 transition-all duration-200 
                    ${isNotesOpen ? 'bg-yellow-50 border-yellow-200 text-yellow-800' : 'bg-gray-50 border-gray-100 text-gray-600 hover:bg-gray-100'}
                  `}
                >
                    <span className="flex items-center gap-2">
                        <svg className={`w-4 h-4 ${isNotesOpen ? 'text-yellow-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        Session Notes
                    </span>
                    <svg className={`w-4 h-4 transition-transform duration-300 ${isNotesOpen ? 'rotate-180 text-yellow-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </button>
                
                {isNotesOpen && (
                    <div className="mt-2 p-3 bg-yellow-50 border border-yellow-100 rounded-lg text-xs text-yellow-900 leading-relaxed">
                        {schedule.notes}
                    </div>
                )}
            </div>
